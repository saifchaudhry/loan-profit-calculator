class LoanApplicationMailer < ApplicationMailer
  ...
  def estimated_profit_mail(loan_application_id)
    @loan_application = LoanApplication.find_by_id loan_application_id
    attachments["loan_application_#{@loan_application.id}.pdf"] = ::WickedPdf.new.pdf_from_string(
      render_to_string(pdf: "loan_application_#{@loan_application.id}", 
                       template: 'loan_application/_estimated_profit',
